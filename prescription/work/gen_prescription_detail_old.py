...
from prescription.work import OUTPUT_DIR, INPUT_DIR
...
insert into prescription_detail(prescription_id, sku_id, `name`, drug_name, num, unit, `usage`, per_amount, 
...
#                         """)
        ...
        for pres_detail in origin_pres_detail_list:
            ...
            if online_drug:
                ...
                detail_dict['drug_name'] = quote_or_MYSQL_NULL(online_drug['drug_name'])
                detail_dict['num'] = MYSQL_NULL if not online_drug['num'] else online_drug['num']
                detail_dict['unit'] = quote_or_MYSQL_NULL(online_drug['unit'])
                detail_dict['usage'] = quote_or_MYSQL_NULL(online_drug['usage'])
                detail_dict['per_amount'] = MYSQL_NULL if not online_drug['per_amount'] else online_drug['per_amount']
                detail_dict['times'] = MYSQL_NULL if not online_drug['times'] else online_drug['times']
                detail_dict['remark'] = quote_or_MYSQL_NULL(online_drug['remark'])
                detail_dict['usage_frequency_code'] = MYSQL_NULL if not online_drug['usage_frequency_code'] else \
                    online_drug['usage_frequency_code']
                detail_dict['usage_frequency_name'] = quote_or_MYSQL_NULL(online_drug['usage_frequency_name'])
                detail_dict['per_num'] = quote_or_MYSQL_NULL(online_drug['per_num'])
                detail_dict['per_unit'] = quote_or_MYSQL_NULL(online_drug['per_unit'])
                detail_dict['dose'] = MYSQL_NULL if not online_drug['dose'] else online_drug['dose']
                ...
                detail_dict['remark'] = MYSQL_NULL
                detail_dict['usage_frequency_code'] = MYSQL_NULL
                detail_dict['usage_frequency_name'] = quote_or_MYSQL_NULL(random_pick(['1次/天', '3次/天', '2次/天'], [0.5, 0.4, 0.1]))
                detail_dict['per_num'] = MYSQL_NULL
                detail_dict['per_unit'] = MYSQL_NULL
                detail_dict['dose'] = MYSQL_NULL
        ...
        # , ym_date = row[0]
    logging.info('{this_year} miss match pres detail {drug_online_miss_match_number}')
